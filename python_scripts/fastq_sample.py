...
if __name__=="__main__":
	#paras set 
	parser = argparse.ArgumentParser(description = 'A program for random sample reads in fastq file')
	...
	parser.add_argument('--sample_num', help='choose how many items each time, default=10000', type=int, default=1000)
	...
	#multiprocessing
	p = Pool()
	for dat_f in args.input:
		#for i in range(10): print(total_reads[i])
		p.apply_async(my_sample, args = (args.sample_num, args.sample_cycle, args.replace, dat_f))
	p.close()
	p.join()
