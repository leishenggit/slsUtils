...
if __name__ == '__main__':
	...
	with gzip.open(input_file, "rt") as handle:
		for r in SeqIO.parse(handle, 'fastq'): read_count[r.id] += 1
		single = (r for r in SeqIO.parse(handle, 'fastq') if read_count[r.id] == 1 )
		count = SeqIO.write(single, output_s, "fastq")
		print("Saved %i records from %s to %s" % (count, input_file, output_s) )
		paired = (r for r in SeqIO.parse(handle, 'fastq') if read_count[r.id] == 2 )
